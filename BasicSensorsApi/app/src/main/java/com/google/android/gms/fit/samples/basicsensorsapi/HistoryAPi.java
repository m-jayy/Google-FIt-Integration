...
public class HistoryAPi extends AppCompatActivity {
    ...
    private static Date MyBuckettDate;
...
//                        .addDataType(DataType.AGGREGATE_HEART_RATE_SUMMARY, FitnessOptions.ACCESS_READ)
                    ...
                    //    .addDataType(HealthDataTypes.AGGREGATE_BODY_TEMPERATURE_SUMMARY, FitnessOptions.ACCESS_READ)
                        //////////////////////
...
//        long startTime = cal.getTimeInMillis();
           ...
           cal.set(Calendar.MINUTE, 0); //so it get java.text.DateFormat dateFormat = getDateInstance();
           long startTime = cal.getTimeInMillis();
...
//
                       ...
                       //
//                        .aggregate(HealthDataTypes.TYPE_BODY_TEMPERATURE, HealthDataTypes.AGGREGATE_BODY_TEMPERATURE_SUMMARY)
    ...
    public static void printData(DataReadResponse dataReadResult) {
        ...
        if (dataReadResult.getBuckets().size() > 0) {
            ...
            for (Bucket bucket : dataReadResult.getBuckets()) {
                ...
                MyBuckettDate = calendar.getTime();
    ...
    //    Log.i(TAG, "Data returned for Data type: " + dataSet.getDataType().getName());
        ...
        for (DataPoint dp : dataSet.getDataPoints()) {
          ...
          DataType type = dp.getDataType();
          Log.i(TAG, "Data point:");
          if(type.equals(DataType.TYPE_WEIGHT))
            {
                 Log.i(TAG, "\tType: " + "Weight");
                  Log.i(TAG, "\tDate: " +getDate(dp.getTimestamp(TimeUnit.MILLISECONDS), "dd/MM/yyyy hh:mm:ss.SSS") );
          ...
          else if     (type.equals(HealthDataTypes.TYPE_BLOOD_PRESSURE)) {
                  Log.i(TAG, "\tType: " + "Blood Pressure");
                    Log.i(TAG, "\tDate: " +getDate(dp.getTimestamp(TimeUnit.MILLISECONDS), "dd/MM/yyyy hh:mm:ss.SSS") );
                    Log.i(TAG, "\tStart: " + dateFormat.format(dp.getStartTime(TimeUnit.MILLISECONDS)));
                    Log.i(TAG, "\tEnd: " + dateFormat.format(dp.getEndTime(TimeUnit.MILLISECONDS)));
                    for (Field field : dp.getDataType().getFields()) {
                        if( field.getName().equalsIgnoreCase("blood_pressure_systolic"))
                        {
                            Log.i(TAG, "\tsystolic Value: " + dp.getValue(field));
                        }
                        else if( field.getName().equalsIgnoreCase("blood_pressure_diastolic"))
                              {
                                  Log.i(TAG, "\tdiastolic Value: " + dp.getValue(field));
                              }
                    }

          }
          else if     (type.equals(DataType.TYPE_HEIGHT)) {
                  Log.i(TAG, "\tType: " + "HEIGHT");
                    Log.i(TAG, "\tDate: " +getDate(dp.getTimestamp(TimeUnit.MILLISECONDS), "dd/MM/yyyy hh:mm:ss.SSS") );
                    Log.i(TAG, "\tStart: " + dateFormat.format(dp.getStartTime(TimeUnit.MILLISECONDS)));
                    Log.i(TAG, "\tEnd: " + dateFormat.format(dp.getEndTime(TimeUnit.MILLISECONDS)));
                    for (Field field : dp.getDataType().getFields()) {
           ...
           else if     (type.equals(DataType.TYPE_HEART_RATE_BPM)) {
                   Log.i(TAG, "\tType: " + "HEART RATE");
                     Log.i(TAG, "\tDate: " +getDate(dp.getTimestamp(TimeUnit.MILLISECONDS), "dd/MM/yyyy hh:mm:ss.SSS") );
                     Log.i(TAG, "\tStart: " + dateFormat.format(dp.getStartTime(TimeUnit.MILLISECONDS)));
                     Log.i(TAG, "\tEnd: " + dateFormat.format(dp.getEndTime(TimeUnit.MILLISECONDS)));
                     for (Field field : dp.getDataType().getFields()) {
            ...
            else if     (type.equals(HealthDataTypes.TYPE_BODY_TEMPERATURE)) {
                    Log.i(TAG, "\tType: " + "BODY TEMPERATURE");
                      Log.i(TAG, "\tDate: " +getDate(dp.getTimestamp(TimeUnit.MILLISECONDS), "dd/MM/yyyy hh:mm:ss.SSS") );
                      Log.i(TAG, "\tStart: " + dateFormat.format(dp.getStartTime(TimeUnit.MILLISECONDS)));
                      Log.i(TAG, "\tEnd: " + dateFormat.format(dp.getEndTime(TimeUnit.MILLISECONDS)));
                      for (Field field : dp.getDataType().getFields()) {
    ...
    public static String getDate(long milliSeconds, String dateFormat)
    {
        ...
        SimpleDateFormat formatter = new SimpleDateFormat(dateFormat);// [END parse_dataset]
